...
d6eプラットフォームでは「STF（State Transition Function）」と呼ばれるデータ処理の部品を使って業務を自動化します。STFのプログラムは

- Dockerイメージ
- JavaScriptコード

のいずれかを実行できる仕組みになっており、難易度と柔軟性が高いDockerイメージを開発するのがこのガイドのゴールです。
JavaScriptに関してはd6eのAIエージェントに指示するだけで簡単にコードの記述やセットアップが完了するためこのガイドでは触れません。
